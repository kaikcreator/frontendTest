...
describe('TargetAudienceComponent', () => {
  ...
  let testTargetAudience = new TargetAudiance({
    languages: ['ES', 'EN'],
    genders: ['M', 'F'],
    age: {min:0, max:100},
    locations:['Spain'],
  ...
  it('should display a card with several items if there`s some targetAudience', () => {
    testHostComponent.testTargetAudience = testTargetAudience;
    fixture.detectChanges();
    let content = componentDe.nativeElement.querySelector('mat-card-content');
    expect(content.children.length).toEqual(6);

    //check app-labeled-chips content
    let labeledChips = fixture.debugElement.queryAll(By.directive(LabeledChipsComponent));
    ...
    let audienceRange = componentDe.nativeElement.querySelector('.target-audience-range');
